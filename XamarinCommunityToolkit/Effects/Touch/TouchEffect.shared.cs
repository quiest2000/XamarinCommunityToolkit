...
namespace Xamarin.CommunityToolkit.Effects
{
	public class TouchEffect : RoutingEffect
	{
		...
		{
			get => GetUserInteractionState(Element);
			internal set => SetUserInteractionState(Element, value);
		}
		...
		public int RippleCount => GetRippleCount(Element);
		...
		internal void RaiseStateChanged()
		{
			ForceUpdateState();
			HandleLongPress();
			StateChanged?.Invoke(Element, new TouchStateChangedEventArgs(State));
		}

		internal void RaiseUserInteractionStateChanged()
			=> InteractionStatusChanged?.Invoke(Element, new TouchInteractionStatusChangedEventArgs(UserInteractionState));

		internal void RaiseStatusChanged()
			=> StatusChanged?.Invoke(Element, new TouchStatusChangedEventArgs(Status));

		internal void RaiseHoverStateChanged()
		{
			ForceUpdateState();
			HoverStateChanged?.Invoke(Element, new HoverStateChangedEventArgs(HoverState));
		}

		internal void RaiseHoverStatusChanged()
			=> HoverStatusChanged?.Invoke(Element, new HoverStatusChangedEventArgs(HoverStatus));

		internal void RaiseCompleted()
			=> Completed?.Invoke(Element, new TouchCompletedEventArgs(CommandParameter));
